...
**Dispatch prompt discipline (load-bearing).** When dispatching *any* specialist subagent (snapshot evaluator, argument auditor, citation auditor, research methods reviewer, conformance scanner, etc.), the prompt MUST include a literal line of the form `Output path: <path>` — not a markdown header, not "Write your output to:" prose, not a paraphrase. The PostToolUse hook (`tier1-verify-dispatch.sh`) extracts the expected path by grep-matching this literal at the start of a line (optional bullet, bold, or backtick decoration is tolerated; no other form matches). A dispatch whose prompt lacks this literal line still produces the artifact but logs `expected_path: null`, which the Tier 1 manifest check treats as a fabrication signal even when the dispatch is legitimate. Pass the phase-skill's brief through verbatim — the `Output path:` line in each phase skill's dispatch subsection is in the correct format; reformatting it breaks extraction. Subagents themselves MUST NOT write to `dispatch-log.jsonl` to "fix" a missing entry — that is a trust-model violation. Re-dispatch with a corrected prompt instead.
...
**BUILD mode:** (optional; `auto` or `gated`; declared by practitioner at BUILD entry; absent defaults to `gated` — per ADR-091)
...
- **`**BUILD mode:**`** — per ADR-091 (v0.8.5), `auto` or `gated`. Declares the BUILD pattern for this cycle: `gated` (default; per-scenario-group EPISTEMIC GATES with practitioner in the loop) or `auto` (autonomous execution after high-level direction; stewardship self-administered; review concentrated at start-and-end). Absence defaults to `gated`. The `/rdd-build` skill reads the field at phase entry and adapts. The mode-selection axes (mechanical-vs-generative work character, practitioner availability, cycle stakes, stewardship locality) are judgment-applied; cycle character determines the right mode. Mid-phase shifts are a small edit to this field. Auto mode does not catch design-alternative examination or scoping-judgment surfacing — those are gated-mode capabilities; practitioners running auto mode know what they are accepting.
- **`**In-progress phase:**`** — per v0.8.2, set by the orchestrator when entering a phase; removed when the agent is ready to invite the phase-exit advisory. The value names the phase in canonical lowercase (`research`, `discover`, `model`, `decide`, `architect`, `build`, `play`, `synthesize`). While the field is present on the top entry and names the current phase, the Stop hook suppresses the per-phase manifest advisory and emits a one-time notice per session. Removing the field invites the next Stop-hook fire to emit the advisory if anything is missing. Per v0.8.3 the manifest advisory never blocks — it surfaces missing artifacts as a stdout message that the agent and user can read and act on. The In-progress phase mechanism's role is to keep that advisory quiet during phase work, not to gate correctness.
...
**Phase entry and exit (v0.8.2 + v0.8.3).** The orchestrator manages the `**In-progress phase:**` field across each phase transition:

1. **On phase entry** (when dispatching to a phase skill): add `**In-progress phase:** <phase>` to the active entry, where `<phase>` is the canonical lowercase name. While set and matching the current phase, the Stop hook suppresses the per-phase manifest advisory.
2. **During the phase**: the field stays set. Phase work proceeds (research loops, gate conversations, specialist subagent dispatches, etc.). The hook is silent on the manifest check.
...
4. **Invite the advisory**: the next Stop fires the manifest check. If all obligations are met, the hook is silent. If not, the hook emits a stdout advisory listing what is missing — address the gaps before advancing.
5. **On clean check**: advance `**Current phase:**` to the next phase and add `**In-progress phase:** <next-phase>` in the same edit. The cycle resumes in the new phase.

The manifest check is **advisory-only as of v0.8.3** — it never blocks. The agent is responsible for actually addressing missing artifacts before advancing; the methodology's real defenses are skill-text guidance, the dispatch-log compound check (also advisory in v0.8.3), and architectural isolation of audit subagents. The In-progress phase mechanism's role is to suppress advisory noise during phase work, not to gate correctness.
