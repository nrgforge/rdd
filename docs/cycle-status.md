...
- **WP-E** (BUILD skill Step 5.5 + Applicability Check stewardship) — implied-dep on WP-C (criteria-table format now established) and WP-D (fitness-property format — stubbable if WP-D is delayed).
- **WP-G** (CONFORM skill cycle-shape audit) — hard-dep on WP-A (satisfied); could ship any time.
- **WP-H** (integration + release) — hard-dep on A–G.

The recommended TS-1-first sequencing has shipped A + B. TS-2 (skill-side) has shipped C. WP-D / WP-F are next independent starts. WP-E now has its criteria-table format input from WP-C; fitness-property input from WP-D remains stubbable if WP-D is delayed. WP-G sits at TS-1 completion — small utility, hard-dep satisfied.
