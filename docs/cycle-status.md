...
All seven dependency WPs (A–G) have shipped. WP-H is the final WP before Cycle 016 BUILD phase exit. Before starting WP-H, the Stop hook enforcement needs to be re-enabled by removing the `**Paused:**` field on the active entry (Cycle 016 was paused via ADR-072 to avoid mid-phase enforcement gaps during WP-B; with the remaining work being verification + release housekeeping rather than substantive amendments, enforcement should resume before WP-H to catch any integration drift).
