...
| Gates are engagement-blind | All EPISTEMIC GATE sections in skill files | Adaptive gates that read the cycle's conversation for engagement signals and calibrate the response — teaching, probing, challenging, or clarifying based on how the user engaged | Engagement-blind protocol | Implement Attend-Interpret-Decide cycle in gate protocol across all skills |
| No self-explanation for new users | Orchestrator skill assumes user knows what RDD is | New users need a brief, adaptive introduction to the methodology — what it is, why it exists, how to start | Missing utility | Implement `/rdd-about` skill |
