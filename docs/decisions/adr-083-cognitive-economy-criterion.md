...
- **The Outcome Test, in-place-first default, and bolt-on / felt-additional signal:** practitioner-articulated at the discover → model gate (recorded in `docs/housekeeping/gates/017-discover-gate.md` and the discover susceptibility snapshot). Driver-derived; user-surfaced through belief-mapping at the gate.
- **The variability-across-corpora as per-project judgment:** practitioner-articulated at the model → decide gate (recorded in `docs/housekeeping/gates/017-model-gate.md`). Driver-derived; user-surfaced.
