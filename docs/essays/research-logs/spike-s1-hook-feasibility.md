...
**Expected artifact:** `./.rdd/audits/research-design-review-NNN.md`.
...
1. On every `PostToolUse` where `tool_name == "Agent"` and `tool_input.subagent_type == "research-methods-reviewer"`, extract the expected output path from `tool_input.prompt` (a grep or regex for `./.rdd/audits/research-design-review-*.md`).
2. `test -f` the path.
...
**Expected artifact:** `./.rdd/audits/citation-audit-NNN.md`.
...
**Expected artifact:** `./.rdd/audits/argument-audit-NNN.md` (research) or `./docs/decisions/audits/argument-audit-NNN.md` (decide).
