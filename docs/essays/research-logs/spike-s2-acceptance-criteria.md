...
Expected path: `.rdd/audits/research-design-review-{cycle}.md`.
...
Expected path: `.rdd/audits/citation-audit-{cycle}.md`.
...
Expected paths: `.rdd/audits/argument-audit-{cycle}.md` (research) or `.rdd/audits/argument-audit-decide-{cycle}.md` (decide). The decide variant is distinguished because argument audits on ADRs are a separate dispatch site.
...
Expected path: **proposed convention** `.rdd/audits/susceptibility-snapshot-{cycle}-{phase}.md`. (The current ADR-057 and SKILL.md do not fix a path; S1 flagged this as a softspot. This spike proposes the convention above: cycle number + phase name so one snapshot exists per phase boundary.)
...
- **Per-phase structure.** The top-level `phases` dictionary is keyed by the phase names used in `.rdd/cycle-status.md` (`research`, `discover`, `model`, `decide`, `architect`, `build`, `play`, `synthesize`). The hook looks up the current phase and iterates only that phase's requirements. Adding a new phase means adding a new key.
...
phases:
  ...
  research:
    ...
    required_mechanisms:
      ...
      - mechanism: citation-auditor
        path_template: .rdd/audits/citation-audit-{cycle}.md
        min_bytes: 1500
        required_headers:
      ...
      - mechanism: argument-auditor
        path_template: .rdd/audits/argument-audit-{cycle}.md
        min_bytes: 1500
        required_headers:
  ...
  decide:
    ...
    required_mechanisms:
      - mechanism: argument-auditor
        path_template: .rdd/audits/argument-audit-decide-{cycle}.md
        min_bytes: 1500
        required_headers:
          - "# Argument Audit Report"
          - "## Summary"
        required_fields:
          ...
          - "Issues found:"
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-decide.md
        min_bytes: 400
        required_headers:
          - "# Susceptibility Snapshot"
        required_fields:
  ...
  discover:
    required_mechanisms:
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-discover.md
        min_bytes: 400
  ...
  model:
    required_mechanisms:
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-model.md
        min_bytes: 400
        required_headers: ["# Susceptibility Snapshot"]
        required_fields: ["Phase:"]
  ...
  architect:
    required_mechanisms:
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-architect.md
        min_bytes: 400
        required_headers: ["# Susceptibility Snapshot"]
        required_fields: ["Phase:"]
  ...
  build:
    required_mechanisms:
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-build.md
        min_bytes: 400
        required_headers: ["# Susceptibility Snapshot"]
        required_fields: ["Phase:"]
  ...
  play:
    required_mechanisms:
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-play.md
        min_bytes: 400
        required_headers: ["# Susceptibility Snapshot"]
        required_fields: ["Phase:"]
  ...
  synthesize:
    required_mechanisms:
      - mechanism: argument-auditor
        path_template: .rdd/audits/argument-audit-synthesize-{cycle}.md
        min_bytes: 1500
        required_headers: ["# Argument Audit Report"]
      - mechanism: susceptibility-snapshot-evaluator
        path_template: .rdd/audits/susceptibility-snapshot-{cycle}-synthesize.md
        min_bytes: 400
        required_headers: ["# Susceptibility Snapshot"]
        required_fields: ["Phase:"]
```
...
REPO_ROOT="$(pwd)"
CYCLE_STATUS="${REPO_ROOT}/.rdd/cycle-status.md"
...
**State B coverage (did it fire at all):** **fully caught.** If no file exists at `.rdd/audits/argument-audit-014.md`, the Stop hook's E1 check fails and emits a block naming the missing file. The test harness's Case 2 demonstrates this concretely.
...
>
>   - susceptibility-snapshot-evaluator: expected artifact missing at .rdd/audits/susceptibility-snapshot-014-discover.md
>
...
The spike's hook reads `.rdd/cycle-status.md` for a `**Phase:**` line, but this depends on the project's convention of keeping that file current. If phase transitions lag the status file, the hook checks the wrong phase's requirements. Alternatives:
