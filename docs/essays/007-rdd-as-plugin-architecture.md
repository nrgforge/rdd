...
This matters more than context savings. With 1M token context windows, a citation audit's trace output is not a space crisis. But an audit report that exists only in conversation context is ephemeral — it cannot be reviewed later, referenced by future cycles, or read independently of the session that produced it. An audit report written to `docs/audits/citation-audit-007.md` is addressable, persistent, and inspectable. It becomes part of the artifact trail.
