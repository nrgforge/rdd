...
The implication is that sycophancy resistance needs to be front-loaded, not back-loaded. The phases where the user's framing is established need the strongest structural constraints, not the weakest. This observation is itself produced within the conditions it describes — the gradient and the hierarchy that follows are claims of this analysis, not empirically verified properties of the pipeline. Their validity at the framing level, not just the inferential level, cannot be confirmed from inside the methodology that produced them.

## 7. Toward an Architecture of Genuine Resistance

The research supports a design principle: **genuine structural resistance comes from contact with systems that cannot sycophantically agree, positioned at the points in the pipeline where vulnerability is highest.**
...
The "Ask don't tell" finding (Dubois et al., 2026) suggests one concrete mechanism: making questioning formats the default gate interaction form rather than a special move. If the Inversion Principle's questioning frame were the default rather than one pedagogical option among several, the interaction form itself would constrain the agent toward surfacing tensions rather than extending the user's framing. This is a Level 2 intervention (interaction-form) applied at Level 3 (process-structural) — and the Dubois et al. finding provides evidence that it reduces sycophancy more effectively than behavioral instructions.
...
## 9. What This Research Does Not Resolve
...
**Empirical validation.** No study has tested structured analytic techniques as interface structures for human-AI collaboration. The theoretical case for artifact-grounded resistance is strong; the empirical case is absent. Whether IBIS-structured, Toulmin-structured, or commitment-gated human-AI sessions produce less sycophantic output than free-form conversation is unknown.
