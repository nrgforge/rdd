...
**Role separation between historical record and current state (ADR-074, NEW v13.0):**

ADRs serve the historical-record role: "what was decided, when, why, what alternatives were rejected." Their value is fidelity to the moment of decision; bodies are immutable after acceptance, only the status field and supersession headers may change. `system-design.md`, `ORIENTATION.md`, `domain-model.md`, and `field-guide.md` serve the current-state role: "what is the architecture today, with provenance to the decisions that produced it." Their value is currency. When these roles are separated, a reader asking "what is current?" reads the current-state document; a reader asking "what was decided?" reads the ADR. Each artifact does one job and stays accurate at it. Supersession events trigger a mandatory downstream sweep (ADR-074 Step 3) to keep current-state artifacts pointing to the new authoritative decision.
...
- `product-discovery.md` — stakeholder maps, jobs, value tensions, assumption inversions. Written in user language. The product perspective. Cycle 10 additions: sycophancy propagation patterns, scope transparency framing, and the methodology's own visible limitations as a product quality. Cycle 016 update via DISCOVER added value tensions and assumption inversions covering acceptance criteria layer-match, ADR immutability, scope-adaptive enforcement, and structural-vs-cognitive defense layering.
- `system-design.md` — module decomposition, responsibility allocation, dependency graph, provenance chains. v13.0 Amendment #14: nine module amendments for Cycle 016 (Decide, Architect, Build, Research, Conform, Orchestrator, Tier 1 Phase Manifest Check Hook, Tier 1 Phase Manifest, Research Methods Reviewer Agent), Methodology Seams Concepts responsibility matrix, integration contracts for the new dependency edges. Zero new modules introduced. The technical perspective.
- `roadmap.md` — Cycle 016 active work (8 work packages A–H with classified dependencies and three transition states); Completed Work Log through Cycle 15 (WP-A through WP-E across v0.7.3). The sequencing perspective.

**Tier 3 — Supporting Material (consulted for provenance and depth)**
- `domain-model.md` — concepts, actions, relationships, invariants. Amendment 19: Invariant 8 extended with cognitive resources (Einstellung effect demonstrates a specified lens is not a triggered lens — structural encoding is first-line defense, cognitive-mechanism design is second-line); 9 Cycle 016 concepts added (Acceptance Criteria Table, Criterion-Level Verification, ADR Supersession, Question Isolation, Applicability Check, Non-Interrupting Enforcement, Scope-Adaptive Enforcement, Expand-Outward / Evaluate-Inward, Einstellung Effect). The vocabulary authority.
- `essays/` — research findings (15 essays: 001-014 as before; 015 — Methodology Seams: What Gaps Between Artifacts Reveal About Methodology Design — investigates seven open issues in a single batch cycle, identifies methodology seams as the common pattern, with cognitive literature explaining why these boundaries are vulnerable and structural literature showing where explicit design is needed)
- `decisions/` — ADRs (82 decisions: 001-054 as settled; 055-062 sycophancy resistance architecture; 063-070 structural enforcement for Invariant 8; 071-072 lifecycle composition + cycle-shape declaration; 073-082 Cycle 016 methodology seams interventions — acceptance criteria table, supersession workflow, preservation scenarios, fitness decomposition, applicability check, multi-cycle schema, non-interrupting Stop predicate, scope-adaptive enforcement, grandfathered migration, question-isolation protocol). ADR-072 has an `Updated by ADR-078` header per the new v13.0 supersession workflow.
- `scenarios.md` — refutable behavior specifications (~480 scenarios total; the Cycle 016 additions cover acceptance criteria, supersession, preservation, fitness decomposition, applicability check, multi-cycle, non-interrupting predicate, scope-adaptive enforcement, grandfathered migration, question-isolation — with preservation scenarios applied recursively to the methodology's own additions per ADR-075)
- `interaction-specs.md` — workflow-level specification of how each stakeholder works with the system. Creates the playable surface. Cycle 016 additions: Solo Developer-Researcher running DECIDE extensions; Solo Developer-Researcher initiating RESEARCH question-isolation; Everyday Developer at BUILD pattern-reuse engagement (operative-trigger only — explicit reference); Solo Developer-Researcher operating cycles under multi-cycle schema.
...
**Plugin:** RDD is packaged as a Claude Code plugin (`nrgforge/rdd`) at v0.7.3 with a four-layer architecture: 15 skills (orchestration + composable skill family + utility skills), 8 specialist subagents, 7 cross-cutting hooks (1 PreToolUse, 1 UserPromptSubmit, 2 Stop, 3 PostToolUse), and the self-referential artifact corpus. All skills implemented. Harness Layer infrastructure (per-phase manifest, PostToolUse dispatch logger, Stop hook compound check) is operational in enforcement mode on migrated corpora, and honors cycle-shape declaration (Skipped phases, Paused) per ADR-072. Cycle 016 methodology-seams interventions (ADRs 073–082) are designed and pending BUILD; the multi-cycle Cycle Stack schema, non-interrupting in-progress-gate predicate, applicable_when preconditions, and grandfathered enforcement for legacy cycles ship together with the skill-side amendments.

**Pipeline:** RESEARCH → DISCOVER → MODEL → DECIDE → ARCHITECT → BUILD → [PLAY] → [SYNTHESIZE] → GRADUATE. A conformance audit utility (`/rdd-conform`) operates outside the pipeline and includes a `migrate` subcommand (ADR-070) for the housekeeping migration that enables enforcement mode, and (Cycle 016 pending) a cycle-shape audit operation (ADR-081) that walks pre-ADR-072 cycle-status.md files into the new Cycle Stack format while preserving prose body verbatim.

**Active cycle:** Cycle 016 — Multi-Issue Batch Cycle (Issues #10–#16). RESEARCH ✅ (Essay 015 — Methodology Seams) → DISCOVER ✅ → MODEL ✅ (domain-model.md Amendment 19, 9 concepts, Invariant 8 extended) → DECIDE ✅ (10 ADRs, scenarios + interaction specs added) → **ARCHITECT ✅** (system-design v13.0 Amendment #14, roadmap WPs A–H, this regeneration) → BUILD pending. Susceptibility snapshots at decide → architect and architect → build boundaries both produced one Grounding Reframe each (ADR-078 stack framing examined at gate; ADR-077 aspirational triggers reclassified) — both resolved at the gate. Defect-and-mitigation framing established at the architect → build gate as the proportional-engineering principle for BUILD scope.
...
- **Cycle 15 — Lifecycle Composition in Build Stewardship.** Mini-cycle (DECIDE + BUILD only) amending the build skill with a third integration-risk category (Lifecycle Composition), plus (mid-cycle scope creep) adding cycle-shape declaration to cycle-status.md to close a Stop-hook loop the methodology surfaced on itself. ADR-071 (three-sided catch at design-time, verification-time, review-time anchors) + ADR-072 (Skipped phases and Paused fields in cycle-status.md honored by the Stop hook). Five WPs. Three glossary entries added (Lifecycle Composition, Shared Mutable State, Lifecycle Boundary). Shipped in v0.7.3. Known gap documented: ADR-072's two-field design did not cover the in-progress-gate case — addressed by Cycle 016 ADR-079 with `**In-progress gate:**` field and non-interrupting Stop predicate.
