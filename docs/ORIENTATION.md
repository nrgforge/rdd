...
**Plugin:** RDD is packaged as a Claude Code plugin (`nrgforge/rdd`) at v0.7.2 with a four-layer architecture: 15 skills (orchestration + composable skill family + utility skills), 8 specialist subagents, 7 cross-cutting hooks (1 PreToolUse, 1 UserPromptSubmit, 2 Stop, 3 PostToolUse), and the self-referential artifact corpus. All skills implemented. Harness Layer infrastructure (per-phase manifest, PostToolUse dispatch logger, Stop hook compound check) is operational in enforcement mode on migrated corpora.

**Pipeline:** RESEARCH → DISCOVER → MODEL → DECIDE → ARCHITECT → BUILD → [PLAY] → [SYNTHESIZE] → GRADUATE. A conformance audit utility (`/rdd-conform`) operates outside the pipeline and now includes a `migrate` subcommand (ADR-070) for the one-shot housekeeping migration that enables enforcement mode.

**Completed cycle:** Cycle 10 — Specification-Execution Gap in Prompt-Based Methodology. All six work packages delivered across three releases: WP-A (Harness Layer hooks + manifest), WP-B (Skill-Structure Layer per-phase dispatch), WP-C (User-Tooling Layer gate reflection note), WP-D (orchestrator documentation of Three-Tier Classification, Grounding Reframe Extension, Methodology Scope-of-Claim, Centered-vs-Infrastructure framing) shipped in v0.7.0. WP-E (`/rdd-conform migrate` subcommand + post-migration paths) shipped in v0.7.1. WP-F (behavioral verification + migration dogfood + remediation of nine defects surfaced during live runtime testing) shipped in v0.7.2. ADRs 063-070. Essay 014 (Specification-Execution Gap). Invariant 8 added at MODEL. Domain model Amendment 17. System Design v12.0 Amendment #13.

**What's settled (through ADR-070):**
